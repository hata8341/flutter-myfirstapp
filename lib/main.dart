...
class StartPage extends StatelessWidget {
  ...
  void showPuzzlePage(BuildContext context) {
    Navigator.push(context, MaterialPageRoute(builder: (context) {
      return const PuzzulePage();
    }));
  }
}

class PuzzulePage extends StatefulWidget {
  const PuzzulePage({Key? key}) : super(key: key);

  @override
  _PuzzulePageState createState() => _PuzzulePageState();
}

class _PuzzulePageState extends State<PuzzulePage> {
    ...
    return Scaffold(
      appBar: AppBar(
        ...
        actions: [
          ...
          IconButton(
              onPressed: () => saveTileNumbers(), icon: const Icon(Icons.save)),
        ],
      ),
      body: Padding(
  ...
  bool calcIsCorrect(List<int> numbers) {
    ...
    for (int i = 0; i < correctNumbers.length; i++) {
      if (correctNumbers[i] != numbers[i]) return false;
    }
    return true;
  }
